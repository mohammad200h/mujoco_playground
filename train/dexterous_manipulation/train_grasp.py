...
def save_return_plot(num_steps, metrics):
    times.append(datetime.now())
    x_data.append(num_steps)
    y_data.append(metrics["eval/episode_reward"])
    y_dataerr.append(metrics["eval/episode_reward_std"])
    ...
    plt.savefig('training/training_progress.png')
...
def main():
  ...
  # save_return_plot(metrics, ppo_params["num_timesteps"] * 1.25)
  display_video(frames, fps=1.0 / env.dt / render_every)


  # Save metrics to a JSON file
  # with open('training/training_metrics.json', 'w') as f:
